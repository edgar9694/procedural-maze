...
interface Block {
    position: Position,
    adjacentBlocks: { pastBlock: number | undefined, nextBlock: number | undefined}
    direction: string,
    mesh: Array<boolean> | []
    name: string
}
...
export class MazeGenerator {
    ...
    public sizeLab: number;
    public fixCoord;
    public planePosition;
    public sizeGrid;
    public setDirectionCube: { [key: string]: Array<boolean>} = {
    ...
    // ];
    public setPath: { [key: string]: Array<number>} = {
        left: [0],
        right: [1],
        up: [4],
        down: [5],
    ...
    public geoPathBox;
    public floorMesh =  new THREE.MeshBasicMaterial({color: 0x008000, side: THREE.DoubleSide});
    public wallMesh = new THREE.MeshBasicMaterial({color: 0x808080, side: THREE.DoubleSide});
    public transparentWallMesh = new THREE.MeshBasicMaterial({transparent: true, opacity: 0});
    // public wallGroupMesh = new THREE.Group(); 
    public wallGroupMesh: Array<THREE.Object3D>  = []
    public pathGroup: Array<Block> = [];
    public referenceCone!: THREE.Mesh

    constructor(blocks: number, sizeLab?: number) {
        this.blocks = blocks;
        if(!sizeLab){
            this.sizeLab = blocks
        } else {
     ...
     */
    getRandomArbitrary(max: number, min?: number) {
        if(!min){
            min = 0;
        }
        min = Math.ceil(min);
     ...
     */
    retrieveCoord(block: Block): Array<Array<number>>{
        let b: Array<Array<number>> 
        b = [
            [block?.position.x + this.sizeLab, block?.position.z],
            [block?.position.x - this.sizeLab, block?.position.z ],
            [block?.position.x, block?.position.z + this.sizeLab],
            [block?.position.x, block?.position.z - this.sizeLab]
        ];        
        b = b.filter(b =>
            (b[0] <= (this.sizeGrid - this.fixCoord) && b[0] >= this.fixCoord) && 
            (b[1] <= (this.sizeGrid - this.fixCoord) && b[1] >= this.fixCoord)
        );    
        let newB: Array<Array<number>>  = [];  
        b.forEach(b =>{
    ...
    insertInitialPoint(){
        // initial point
        let block:Block = initialBlockvalue
        let posiblePoints: Array<Array<number>>  = [
        ...
        let box: Array<boolean> = [true, true, false, true, true, true]
    ...
    insertPoint(position: Position, pastBlock:Block, length:number){
        let block:Block =  {
            position:{ x: 0, y: 0, z:0 },
            adjacentBlocks: {
                pastBlock: undefined,
        ...
        let pastBox: Array<boolean> = [true, true, false, true, true, true]
        let pastChangeVar;
    ...
    fixName(pastName:string, newString:string){        
    ...
    generateWallBlock(block: Block) {
        let matArray = [];              
        for (let index = 0; index < block?.mesh.length; index++) {
            let element = block?.mesh[index];
    ...
    setReferencePoint(initialPoint: Block){
        const geo = new THREE.ConeGeometry( 5, 20, 32)
        const material = new THREE.MeshBasicMaterial( {color: 0xffff00} );

        this.referenceCone = new THREE.Mesh(geo, material)
        this.referenceCone.position.set(
            initialPoint.position.x,
            initialPoint.position.y +30,
            initialPoint.position.z
            );
